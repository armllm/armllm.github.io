# Armenia LLM Summer School 2026 — Funding Request Slides

This folder contains a **6-slide** donor-facing deck for the **Armenia LLM Summer School 2026** funding request.

## Files

- `funding-request-slides-2026.html`: the slide deck (printable 16:9 pages).
- `funding-request-metrics.md`: the aggregate metrics used in the deck.

## Export to PDF (recommended)

1. Open `2026/funding-request-slides-2026.html` in a browser.
2. Print → **Save as PDF**.
3. Ensure print settings:
   - **Margins**: None (or minimum)
   - **Background graphics**: On

## Customize before sending to donors

- Update the **funding target / tiers** on Slide 6 (currently examples/placeholders).
- Add a one-line **impact target** (e.g., “X additional scholarships”, “Y additional seats”, “Z recorded lectures”).
- If needed, add a “Proof points” line from the websites (e.g., **70+ participants** highlight and media coverage), which are now summarized in `2026/funding-request-metrics.md`.

